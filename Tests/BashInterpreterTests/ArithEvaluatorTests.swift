...
final class ArithEvaluatorTests: XCTestCase {
    ...
    func testLiteral() throws { XCTAssertEqual(try eval("42"), 42) }
    ...
    func testLogicalNotNonZero() throws { XCTAssertEqual(try eval("!5"),    0) }
    func testBitwiseNot()        throws { XCTAssertEqual(try eval("~0"),   -1) }

    // MARK: Precedence

    func testMulBindsTighterThanAdd() throws {
    ...
    func testPow() throws { XCTAssertEqual(try eval("2 ** 10"), 1024) }
    func testPowRightAssoc() throws {
        // 2 ** 3 ** 2 = 2 ** 9 = 512
        XCTAssertEqual(try eval("2 ** 3 ** 2"), 512)
    }
    func testUnaryMinusLooserThanPow() throws {
        // Bash: -2 ** 2 == -(2**2) == -4
        XCTAssertEqual(try eval("-2 ** 2"), -4)
    }
    func testNegativeExponentThrows() {
        XCTAssertThrowsError(try eval("2 ** -1"))
    }

    // MARK: Comparison

    func testComparisonReturnsZeroOrOne() throws {
    ...
    func testLogicalAndTruthTable() throws {
    ...
    func testLogicalOrTruthTable() throws {
        XCTAssertEqual(try eval("1 || 1"), 1)
    ...
    func testAndShortCircuitsOnFalse() throws {
        let store = Store()
    ...
    func testOrShortCircuitsOnTrue() throws {
        let store = Store()
    ...
    func testTernaryFalse() throws { XCTAssertEqual(try eval("0 ? 10 : 20"), 20) }
    func testTernaryNesting() throws {
        XCTAssertEqual(try eval("1 ? 2 ? 3 : 4 : 5"), 3)
    }
    func testTernaryShortCircuits() throws {
        let store = Store()
    ...
    func testPlainAssignment() throws {
        let store = Store()
    ...
    func testCompoundAssignments() throws {
    ...
    func testBitwiseCompoundAssignments() throws {
    ...
    func testPostfixIncrementReturnsOldValue() throws {
        let store = Store(["x": "5"])
        XCTAssertEqual(try eval("x++", store: store), 5)
        XCTAssertEqual(store.vars["x"], "6")
    }

    func testPrefixIncrementReturnsNewValue() throws {
        let store = Store(["x": "5"])
        XCTAssertEqual(try eval("++x", store: store), 6)
        XCTAssertEqual(store.vars["x"], "6")
    }

    func testPostfixDecrement() throws {
        let store = Store(["x": "5"])
        XCTAssertEqual(try eval("x--", store: store), 5)
        XCTAssertEqual(store.vars["x"], "4")
    }

    func testPrefixDecrement() throws {
        let store = Store(["x": "5"])
    ...
    func testCommaReturnsLast() throws {
        XCTAssertEqual(try eval("1, 2, 3"), 3)
    }

    func testCommaEvaluatesSideEffects() throws {
        let store = Store()
    ...
    func testSignedOverflowWraps() throws {
