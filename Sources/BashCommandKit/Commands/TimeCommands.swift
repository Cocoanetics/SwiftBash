...
public struct TimeCommand: Command {
    ...
    public func run(_ argv: [String], shell: Shell) async throws -> ExitStatus {
        ...
        guard !args.isEmpty else {
            shell.stderr("time: usage: time COMMAND [ARG...]\n")
            return ExitStatus(2)
        }
        let start = Date()
...
public struct TimeoutCommand: Command {
    ...
    public func run(_ argv: [String], shell: Shell) async throws -> ExitStatus {
        ...
        guard args.count >= 2 else {
            shell.stderr("timeout: usage: timeout DURATION COMMAND [ARG...]\n")
            return ExitStatus(2)
        }
        guard let seconds = parseDuration(args[0]) else {
            shell.stderr("timeout: invalid duration: \(args[0])\n")
            return ExitStatus(2)
        }
        ...
        // give us a way to actually kill an in-flight `shell.run`, so
        ...
        return try await withThrowingTaskGroup(of: TimeoutResult.self) { group in
            group.addTask {
                let s = try await shell.run(line)
                return .completed(s)
            }
            group.addTask {
            ...
            for try await result in group {
                ...
                case .timedOut:
                    shell.stderr("timeout: command timed out\n")
